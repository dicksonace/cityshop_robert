...
export default function ProductEngagementStats({
    ...
    const playCount = videoPlays == null ? null : Math.max(0, Number(videoPlays) || 0);
    ...
    return (
        <div
            ...
            aria-label={`${formatCompactCount(viewCount)} views${playCount != null ? `, ${formatCompactCount(playCount)} video plays` : ''}, ${formatCompactCount(likeCount)} likes`}
        >
            ...
            {playCount != null && (
                <>
                    ...
                    <span className="inline-flex items-center gap-1" title={`${playCount.toLocaleString()} video plays`}>
                        <Play className={cn(iconClass, 'text-orange-400')} aria-hidden />
                        <span className="font-medium tabular-nums text-gray-600">{formatCompactCount(playCount)}</span>
                        {size === 'md' && <span className="text-gray-400">plays</span>}
                    </span>
                </>
            )}
            <span className="h-3 w-px bg-gray-200" aria-hidden />
