...
import DeepSearchOverlay from '@/components/shop/deep-search-overlay';
...
export default function ImageSearchUpload({
    ...
    const submit = (file?: File | null) => {
        const selected = file ?? inputRef.current?.files?.[0];
        if (!selected) return;
    ...
    return (
        <>
            <DeepSearchOverlay open={busy} previewUrl={preview} active={busy} />
                ...
                >
                    {preview ? (
                        <div className="flex flex-col items-center gap-4">
                            ...
                            <p className="text-sm text-gray-500">Ready for Deep Search</p>
                        </div>
                    ) : (
                        <div className="text-center">
                            ...
                            <p className="mt-1 text-sm text-gray-500">
                                Take a picture or upload from gallery — we&apos;ll deep-search similar items
                            </p>
                        </div>
                    )}
