...
export default function SellerQrReceive({ qr, amount, reason }: Props) {
    ...
    return (
        <SellerLayout title="My QR" active="wallet">
            <Head title="My QR" />

            <div className="mx-auto max-w-lg space-y-4">
                <Link href={route('seller.wallet')} className="text-sm font-semibold text-orange-600 hover:underline">
                    &larr; Back to wallet
                </Link>

                <div className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-gray-100">
                    {flash?.success && (
                        <p className="mb-4 rounded-xl bg-emerald-50 px-3 py-2 text-sm text-emerald-800">{flash.success}</p>
                    )}
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-gray-500">Buyers scan this to pay you from their wallet</p>
                        </div>
                    </div>
